...
import { Menu, X, LayoutDashboard, Users, GraduationCap, Wallet, Zap, LogOut, Bell, ChevronRight } from "lucide-react";
...
export default function DashboardLayout({ children }) {
  ...
  const currentPage = links.find(l => l.path === pathname)?.name ?? "Dashboard"
      ...
      `}>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="font-extrabold text-white tracking-tight">EduPanel</span>
              <p className="text-xs text-slate-600 mono leading-none mt-0.5">v2.0</p>
            </div>
          </div>
          <button
            onClick={() => setOpen(false)}
        ...
        <div className="px-4 pt-4">
          <div className="flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/20 rounded-xl px-3 py-2">
            ...
            <span className="mono text-xs text-indigo-400 font-medium">{getCurrentMonth()}</span>
        ...
        <nav className="flex-1 px-4 pt-4 space-y-1 overflow-y-auto">
          <p className="text-xs text-slate-600 uppercase tracking-widest mono px-3 mb-2">Menu</p>
          {links.map(link => {
            ...
            return (
              <Link
                ...
                `}
              >
                <Icon size={16} className={isActive ? "text-indigo-200" : "text-indigo-400 group-hover:text-slate-300"} />
                <span>{link.name}</span>
        ...
        <div className="px-4 pb-5 pt-3 border-t border-slate-800">
          <div className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-slate-800 transition-colors cursor-pointer group">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-slate-200 truncate">{admin.name.split(" ").slice(-1)[0]}</p>
              <p className="text-xs text-indigo-400 truncate">{admin.role}</p>
            </div>
            <LogOut size={14} className="text-slate-600 group-hover:text-rose-400 transition-colors shrink-0" />
          </div>
        </div>
      </aside>
        ...
        <header className="shrink-0 bg-slate-900/80 backdrop-blur border-b border-slate-800 px-4 sm:px-6 py-3 flex items-center gap-4">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2.5 bg-slate-800 rounded-xl px-3 py-1.5 cursor-pointer hover:bg-slate-700 transition-colors">
              ...
              <span className="text-sm font-medium text-slate-200">{admin.name.split(" ").slice(0, 2).join(" ")}</span>
              <span className="text-xs text-indigo-400 mono bg-indigo-500/15 text-indigo-400 px-1.5 py-0.5 rounded-md border border-indigo-500/20">admin</span>
            </div>
          </div>
        </header>
