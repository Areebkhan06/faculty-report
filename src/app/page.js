...
export default function Home() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Replace with: router.push("/dashboard") on success
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="flex flex-col items-center mb-8 select-none">
          <div className="relative mb-4">
            <div className="w-16 h-16 rounded-2xl bg-linear-to-br from-indigo-500 to-violet-600 flex items-center justify-center shadow-2xl shadow-indigo-500/30">
              <span className="text-white font-extrabold text-2xl tracking-tighter">EP</span>
            </div>
            {/* Admin shield badge */}
          ...
          <h1 className="text-2xl font-extrabold text-white tracking-tight">EduPanel</h1>
          <p className="text-slate-500 text-xs mt-1 mono">Institution Management System</p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/20 rounded-xl px-4 py-2.5 mb-6">
          ...
          <p className="text-xs text-indigo-300 font-medium">This portal is restricted to <span className="font-bold">administrators</span> only</p>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-7 shadow-2xl shadow-black/50">
          <div className="mb-6">
            ...
            <p className="text-slate-500 text-sm mt-0.5">Enter your credentials to access the dashboard</p>
          </div>
        ...
        <p className="text-center text-xs text-slate-700 mono mt-6">
          © {new Date().getFullYear()} EduPanel · Admin Access Only
        </p>
      </div>
    </div>
  );
